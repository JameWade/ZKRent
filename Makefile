...
OUTPUT?=build/chengtay-chain

BUILD_TAGS?=tendermint
LD_FLAGS = -X github.com/ChengtayChain/ChengtayChain/version.GitCommit=`git rev-parse --short=8 HEAD`
...
build:
	CGO_ENABLED=$(CGO_ENABLED) GO111MODULE=on go build $(BUILD_FLAGS) -tags '$(BUILD_TAGS)' -o $(OUTPUT) ./cmd/tendermint/
.PHONY: build
...
draw_deps:
	...
	@goviz -i github.com/ChengtayChain/ChengtayChain/cmd/tendermint -d 3 | dot -Tpng -o dependency-graph.png
.PHONY: draw_deps
