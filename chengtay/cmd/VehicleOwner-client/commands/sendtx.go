...
func initialize() (privKey ed25519.PrivKeyEd25519) {
	...
	flag.StringVar(&flagKeyDir, "k", workingDir, "the directory which contains 'private.key.ed25519.json' file.")
	{
		...
		if err != nil {
			fmt.Println("please run VehicleOwner-client genkey firstly")
			panic(err)
		}
...
func sendTx(cmd *cobra.Command, args []string) error {
	...
	if args[0] == "ResponseVehicle" {
		for i := 0; i < 1; i++ {
			rawTransaction := randomRawTransaction(privKey, types.TransactionResponseVehicle)
	...
	} else if args[0] == "ReturenTimeoutVehicle" {
		for i := 0; i < 1; i++ {
			rawTransaction := randomRawTransaction(privKey, types.TransactionReturnVehicleTimeout)
			ret, err := sendRawTransaction(client, rawTransaction)
			if err != nil {
				fmt.Println(err.Error())
				// ignore the error
...
func randomResponseVehicleTransactionValue(privKey ed25519.PrivKeyEd25519) (value *types.ResponseVehicleTransactionValue) {
	...
	valueBytesHashSig, err := privKey.Sign([]byte(value.VehicleID))
	if err != nil {
		panic(err)
	}
	value.VehicleIDSignature = valueBytesHashSig
	return value

}
func randomReturnVehicleTimeoutTransactionValue(privKey ed25519.PrivKeyEd25519) (value *types.ReturnVehicleTimeoutTransactionValue) {
	value = &types.ReturnVehicleTimeoutTransactionValue{}
	value.Timestamp = uint64(time.Now().Unix())
	bytes256 := rand.Bytes(256)
	copy(value.Nonce[:256], bytes256[:256])
	value.VehicleID = "1"
	value.ExceededTime = 31
	value.PublicKey = privKey.PubKey().(ed25519.PubKeyEd25519)
	return value
...
func randomRawTransaction(privKey ed25519.PrivKeyEd25519, transactionType uint32) (rawTransaction types.RawTransaction) {
	...
	if transactionType == types.TransactionReturnVehicleTimeout {
		value = randomReturnVehicleTimeoutTransactionValue(privKey)
	} else if transactionType == types.TransactionResponseVehicle {
		value = randomResponseVehicleTransactionValue(privKey)
	}
